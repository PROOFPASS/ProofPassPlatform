...
echo -e "${COLOR_GREEN}[OK] Node.js $(node -v)${COLOR_RESET}"

# Check PostgreSQL
echo -e "${COLOR_YELLOW}[2/10] Verificando PostgreSQL...${COLOR_RESET}"
if ! command -v psql &> /dev/null; then
    echo -e "${COLOR_YELLOW}[WARNING] PostgreSQL no encontrado${COLOR_RESET}"
    echo "Instala PostgreSQL:"
    echo "  macOS: brew install postgresql@14"
    echo "  Ubuntu: sudo apt install postgresql-14"
    exit 1
fi
echo -e "${COLOR_GREEN}[OK] PostgreSQL instalado${COLOR_RESET}"

# Install dependencies
echo -e "${COLOR_YELLOW}[3/10] Instalando dependencias...${COLOR_RESET}"
npm install
echo -e "${COLOR_GREEN}[OK] Dependencias instaladas${COLOR_RESET}"

# Build packages
echo -e "${COLOR_YELLOW}[4/10] Building packages...${COLOR_RESET}"
npm run build:packages
echo -e "${COLOR_GREEN}[OK] Packages compilados${COLOR_RESET}"
...
if psql -lqt | cut -d \| -f 1 | grep -qw proofpass; then
    echo -e "${COLOR_GREEN}[OK] Base de datos 'proofpass' ya existe${COLOR_RESET}"
else
    echo "Creando base de datos 'proofpass'..."
    createdb proofpass 2>/dev/null || psql -c "CREATE DATABASE proofpass;"
    echo -e "${COLOR_GREEN}[OK] Base de datos creada${COLOR_RESET}"
fi
...
EOF
    echo -e "${COLOR_GREEN}[OK] Creado apps/api/.env${COLOR_RESET}"
else
    echo -e "${COLOR_GREEN}[OK] apps/api/.env ya existe${COLOR_RESET}"
fi
...
EOF
    echo -e "${COLOR_GREEN}[OK] Creado apps/platform/.env.local${COLOR_RESET}"
else
    echo -e "${COLOR_GREEN}[OK] apps/platform/.env.local ya existe${COLOR_RESET}"
fi
...
npm run db:generate
npm run db:migrate
echo -e "${COLOR_GREEN}[OK] Migraciones completadas${COLOR_RESET}"

# Run tests
echo -e "${COLOR_YELLOW}[8/10] Ejecutando tests...${COLOR_RESET}"
npm run test:ci
echo -e "${COLOR_GREEN}[OK] Tests pasados${COLOR_RESET}"

# Check test coverage
echo -e "${COLOR_YELLOW}[9/10] Verificando coverage...${COLOR_RESET}"
npm run test:coverage
echo -e "${COLOR_GREEN}[OK] Coverage >= 85%${COLOR_RESET}"

# Summary
echo -e "${COLOR_YELLOW}[10/10] Resumen de setup...${COLOR_RESET}"
echo ""
echo -e "${COLOR_BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${COLOR_RESET}"
echo -e "${COLOR_GREEN}[SUCCESS] Setup completado exitosamente!${COLOR_RESET}"
echo -e "${COLOR_BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${COLOR_RESET}"
echo ""
